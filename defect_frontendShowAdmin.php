...
<head>
    ...
    <!-- Title Page-->
    <title>DEFECT FRONT END SHOW</title>
...
if(isset($_POST['search'])){
	...
	$sql ="SELECT [Defect Mode], WorkWeek, sum(QtyDefect) as count FROM dbo.FrontEndView WHERE Year like '$searchq' AND WorkWeek BETWEEN '$searchq1' AND '$searchq2' and QtyDefect > 0 and [Defect Mode] is not null group by WorkWeek,[Defect Mode]  order by WorkWeek,[Defect Mode]";
...
if(isset($_POST['search'])){
	...
	$searchq1 = $_POST['search1'];
	$searchq2 = $_POST['search2'];
	$sql1 ="SELECT WorkWeek,sum(QtyDefect) as total1,sum(ERAQtyName) as tambah, CAST(ROUND((convert(decimal,((sum(QtyDefect ) *1000000))))/(sum(ERAQtyName)), 2) AS NUMERIC(20,2)) as result FROM dbo.FrontEndView WHERE Year like '%$searchq%' AND WorkWeek BETWEEN '$searchq1' AND '$searchq2' group by WorkWeek order by WorkWeek";
	
	$stmt1 = sqlsrv_query( $conn, $sql1 );
	
$sql11 ="SELECT WorkWeek,sum(QtyDefect) as total1 FROM dbo.FrontEndView WHERE Year like '%$searchq%' AND WorkWeek BETWEEN '$searchq1' AND '$searchq2' and [Defect Mode] is not null group by WorkWeek order by WorkWeek";
...
if(isset($_POST['search'])){
	...
	$searchq1 = $_POST['search1'];
	$searchq2 = $_POST['search2'];
	$sql2 ="SELECT [Defect Mode], WorkWeek, sum(QtyDefect) as count ,LotNo,Phase,Machine, Package FROM dbo.FrontEndView WHERE Year like '$searchq' AND WorkWeek BETWEEN '$searchq1' AND '$searchq2' and QtyDefect > 0 and [Defect Mode] is not null group by WorkWeek,[Defect Mode],Phase,Machine,LotNo, Package order by WorkWeek,[Defect Mode]";
